...
app.post("/webhook", async(req,res)=>{
  ...
  try {
    ...
    if(session.state==="location"){
      ...
      // Store localized names in order for multi-language support
      const baseOrder = {
      ...
      const TLF = LANGS[firstTechLang] || LANGS.ar;
      const {sName:lSvcName, tName:lTypeName} = localizeOrder(service.name, selectedType.name, service, selectedType, firstTechLang);
      await sendButtons(techPhone,
        TLF.newOrder(orderId, lSvcName, lTypeName, partsText, totalPrice),
...
// Gets the name in tech's language from Firebase object
// Supports: name (ar), nameEn, nameUr, nameBn, nameHi
function getLocalizedName(obj, lang) {
  if(!obj) return "";
  const map = {
    ar: obj.name || obj.nameAr || "",
    en: obj.nameEn || obj.name || "",
    ur: obj.nameUr || obj.name || "",
    bn: obj.nameBn || obj.name || "",
    hi: obj.nameHi || obj.name || "",
  };
  return map[lang] || obj.name || "";
}

// Get localized service/type name for tech notification
function localizeOrder(serviceName, typeName, serviceObj, typeObj, lang) {
  const sName = (serviceObj && getLocalizedName(serviceObj, lang)) || serviceName || "";
  const tName = (typeObj   && getLocalizedName(typeObj,   lang)) || typeName   || "";
  return {sName, tName};
}
...
app.post("/admin/assign", async(req,res)=>{
  try {
    ...
    const {sName:aSvcName,tName:aTypeName} = localizeOrder(order.serviceName,order.type||"",order.serviceObj,order.typeObj,tl);
    await sendButtons(techPhone,
      TL.newOrder(order.orderId||orderId, aSvcName, aTypeName, partsText, order.totalPrice||0),
...
app.listen(process.env.PORT||3000,()=>console.log("✅ TAQA Bot running"));const express     = require("express");
const axios       = require("axios");
const admin       = require("firebase-admin");
const { v4: uuidv4 } = require("uuid");
const PDFDocument = require("pdfkit");
